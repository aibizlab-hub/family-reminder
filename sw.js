  ...
  event.waitUntil(
    caches.keys()
      ...
      .then((clients) => clients.forEach((c) => {
        ...
        try {
          if (c.url && c.url.indexOf(self.location.origin + '/family-reminder') === 0) c.navigate(c.url);
        } catch (e) {}
      }))
  );
});
...
self.addEventListener('push', (event) => {
  let data = { title: 'HomeMemo', body: '你有新的提醒！', icon: '/family-reminder/icon-192.png', badge: '/family-reminder/icon-72.png', tag: 'reminder', url: '/family-reminder/' };
...
self.addEventListener('notificationclick', (event) => {
  ...
  const url = event.notification.data.url || '/family-reminder/';
